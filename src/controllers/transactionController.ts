...
const validateInputs = (inputs: MakeTransactionInputs): string => {
	...
	if (channel_user_id.length > 15 || (to.startsWith("0x") && !Number.isNaN(parseInt(to, 10)) && to.length <= 15)) {
...
const executeTransaction = async (from: IUser, to: IUser | { wallet: string }, token: string, amount: string, chain_id: number) => {
	...
	*/
	...
	try {
		...
		await sendOutgoingTransferNotification(from.phone_number, toNumber, amount, token, result.transactionHash);
	} catch (error) {
...
export const makeTransaction = async (
	...
	try {
		...
		if (!fromUser) {
			return await reply.status(400).send({ message: "Debes tener una wallet creada para poder transferir" });
		}
		...
		return await reply.status(200).send({ message: "Transaccion en progreso... Esto puede tardar unos minutos." });
	} catch (error) {
