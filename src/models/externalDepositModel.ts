...
const externalDepositSchema = new Schema<IExternalDeposit>({
  ...
  provider: { type: String, required: true, enum: ['alchemy', 'thegraph'], default: 'alchemy' },
  status: { type: String, required: true, enum: ['observed', 'finalized'], default: 'observed' }
});
...
externalDepositSchema.index({ status: 1 });

export const ExternalDepositModel = model<IExternalDeposit>('ExternalDeposit', externalDepositSchema, 'external_deposits');
