...
async function validateSwapPrerequisites(
  ...
  try {
    ...
    const uniswapFactoryABI = ['function getPool(address,address,uint24) view returns (address)'];
    ...
    Logger.info(
      ...
      `TokenOut whitelisted: ${outWhitelisted ? '✅' : '❌'}`
    );
    ...
    Logger.info(
      'validateSwapPrerequisites',
      `TokenIn: ${tokenInSymbol} (${tokenInDecimals} decimals)`
    );
    Logger.debug(
      'validateSwapPrerequisites',
      `TokenOut: ${tokenOutSymbol} (${tokenOutDecimals} decimals)`
    );
    ...
    Logger.info(
      'validateSwapPrerequisites',
      `Required balance: ${ethers.utils.formatUnits(amountInBN, tokenInDecimals)} ${tokenInSymbol}`
    );
    Logger.info(
      'validateSwapPrerequisites',
      `Current balance: ${ethers.utils.formatUnits(balance, tokenInDecimals)} ${tokenInSymbol}`
    );
    ...
    if (
      inPriceFeed === ethers.constants.AddressZero ||
      outPriceFeed === ethers.constants.AddressZero
    ) {
    ...
    try {
      ...
      Logger.info(
        'validateSwapPrerequisites',
        `Price of ${tokenOutSymbol}: ${outPrice.toString()}`
      );
      ...
      Logger.error(
        'validateSwapPrerequisites',
        `ERROR getting prices: ${(error as Error).message} ❌`
      );
      return false;
    }
    ...
    Logger.info(
      'validateSwapPrerequisites',
      `Current allowance: ${ethers.utils.formatUnits(allowance, tokenInDecimals)} ${tokenInSymbol}`
    );
    ...
    try {
      ...
      if (poolAddress === ethers.constants.AddressZero) {
        ...
        Logger.error(
          'validateSwapPrerequisites',
          'A pool needs to be created before swaps can be made'
        );
        return false;
      }
      ...
      Logger.info(
        'validateSwapPrerequisites',
        `Pool found: ${poolAddress} (fee: ${feeTier / 10000}%)`
      );
      ...
      Logger.debug(
        'validateSwapPrerequisites',
        `Current sqrt price: ${slot0.sqrtPriceX96.toString()}`
      );
      Logger.debug('validateSwapPrerequisites', `Current tick: ${slot0.tick}`);
    } catch (error: unknown) {
      Logger.error(
        'validateSwapPrerequisites',
        `ERROR verifying pool: ${(error as Error).message} ❌`
      );
      return false;
    }
    ...
    Logger.info(
      'validateSwapPrerequisites',
      `Sufficient balance: ${balance.gte(amountInBN) ? '✅' : '❌'}`
    );
    Logger.info(
      'validateSwapPrerequisites',
      `Sufficient allowance: ${allowance.gte(amountInBN) ? '✅' : '❌'}`
    );
