...
async function createProxy(
  ...
  try {
    ...
    if (
      (error as Error).message.includes('gas') ||
...
async function main() {
  try {
    ...
    // Create the proxy
    const proxyAddress = await createProxy(provider, signer, FACTORY_ADDRESS, ownerAddress);
    ...
    if (proxyAddress) {
      ...
      Logger.info(
        'main',
        `Network: ${(await provider.getNetwork()).name} (${(await provider.getNetwork()).chainId})`
      );
      Logger.info('main', '=====================================================');
      Logger.info(
        'main',
        'Add this address to your .env as PROXY_ADDRESS to use it in other scripts'
      );
