...
describe('generateOnRampLink', () => {
  it('should generate a valid on-ramp link with wallet address', () => {
    ...
    const expectedLink = `${ONRAMP_BASE_URL}?appId=${ONRAMP_APP_ID}&coinCode=${ONRAMP_DEFAULT_COIN_CODE}&network=${mockNetwork}&walletAddress=${mockWalletAddress}`;
    ...
    expect(expectedLink).toContain(`network=${mockNetwork}`);
    expect(expectedLink).toContain(`walletAddress=${mockWalletAddress}`);
  });

  it('should validate link format', () => {
    const mockWalletAddress = '0x1234567890123456789012345678901234567890';
    const link = `${ONRAMP_BASE_URL}?appId=${ONRAMP_APP_ID}&coinCode=${ONRAMP_DEFAULT_COIN_CODE}&network=${mockNetwork}&walletAddress=${mockWalletAddress}`;
