...
import * as mongoPolymarketService from '../../src/services/mongo/mongoPolymarketService';
import * as notificationService from '../../src/services/notificationService';
import * as polymarketService from '../../src/services/polymarket';
import * as polymarketPurchaseService from '../../src/services/polymarket/polymarketPurchaseService';
...
vi.mock('../../src/services/polymarket/polymarketPurchaseService', () => ({
  claimWinningPositions: vi.fn(),
  executePurchase: vi.fn()
}));

vi.mock('../../src/services/mongo/mongoPolymarketService', () => ({
  createOrder: vi.fn(),
...
vi.mock('../../src/services/mongo/mongoBlockchainService', () => ({
  mongoBlockchainService: {
    getNetworkConfig: vi.fn().mockResolvedValue({ chainId: 534351 })
  }
}));

vi.mock('../../src/services/secService', () => ({
  secService: {
    get_up: vi.fn().mockReturnValue('mock-private-key')
  }
}));
...
describe('polymarketController - Terms and Conditions Endpoints', () => {
  ...
  describe('polymarketPurchase - SELL at boundary price (market resolution states)', () => {
    const mockUser = {
      phone_number: '5491122334455',
      settings: { notifications: { language: 'en' } },
      wallets: [{ wallet_proxy: '0xProxy' }],
      polymarket_account: { polygon_address: '0xPolygon' }
    };
    ...
    const makeSellRequest = (price: number) =>
      ({
        body: {
          channel_user_id: '5491122334455',
          token_id: 'token-123',
    ...
    beforeEach(() => {
      ...
      vi.mocked(polymarketService.hasAcceptedCurrentTerms).mockReturnValue(true);
      vi.mocked(polymarketService.getMarketByClobTokenId).mockResolvedValue({
        conditionId: '0xCondition'
      } as any);
      vi.mocked(mongoPolymarketService.createPurchase).mockResolvedValue({} as any);
      vi.mocked(mongoPolymarketService.updatePurchaseStatus).mockResolvedValue({} as any);
    });

    it('claims and succeeds when the market has resolved on-chain', async () => {
      vi.mocked(polymarketPurchaseService.claimWinningPositions).mockResolvedValueOnce({
        claimed: 1,
        txHash: '0xhash'
      });

      const result = await polymarketPurchase(makeSellRequest(0.9995), makeReply());

      expect(result).toMatchObject({ status: 'success', data: { claimed: 1 } });
    });

    it('returns a clear MARKET_PENDING_RESOLUTION error when the market is closed but not yet resolved on-chain', async () => {
      vi.mocked(polymarketPurchaseService.claimWinningPositions).mockResolvedValueOnce({
        claimed: 0,
      ...
      const mockReply = makeReply();
      const result = await polymarketPurchase(makeSellRequest(0.9995), mockReply);

      expect(mockReply.status).toHaveBeenCalledWith(409);
      expect(result).toMatchObject({
        status: 'error',
        data: { code: 'MARKET_PENDING_RESOLUTION' }
      });
      expect((result as any).data.message).toContain('awaiting final resolution');
      expect((result as any).data.message).not.toContain('no claimable winning positions');
    });

    it('falls back to the generic message when nothing is claimable and the market is not pending resolution', async () => {
      vi.mocked(polymarketPurchaseService.claimWinningPositions).mockResolvedValueOnce({
        claimed: 0,
      ...
      expect((result as any).data.message).toContain('no claimable winning positions were found');
    });
  });
});
